...
class Cosmos_Breadcrumb {
	...
	public function get_breadcrumb() {
		return apply_filters( 'cosmos_get_breadcrumb', $this->crumbs, $this );
	}
	...
	public function generate() {
		$conditionals = array(
			...
			'is_date',
			'is_tax',
			'cosmos_is_custom_post_type_archive',
		);
		if ( ( ! is_front_page() && ! ( is_post_type_archive() ) ) || is_paged() || cosmos_is_custom_post_type_archive() ) {
			foreach ( $conditionals as $conditional ) {
				if ( call_user_func( $conditional ) ) {
					if( $conditional == 'cosmos_is_custom_post_type_archive'){
						$crumbs_func = 'post_type_archive';
					} else {
	...
	private function add_crumbs_404() {
		$this->add_crumb( esc_html__( 'Error 404', 'cosmos' ) );
	}
	...
	private function add_crumbs_single( $post_id = 0, $permalink = '' ) {
		...
		if ( 'post' != get_post_type( $post ) ) {
			...
			$cat = '';
			$post_options = get_post_meta( $post->ID, 'cosmos_post_options', true);
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( esc_html__( 'Posts tagged', 'cosmos' ).sprintf( ' &ldquo;%s&rdquo;', single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_author() {
		...
		$this->add_crumb( esc_html__( 'Author', 'cosmos' ).': '.$userdata->display_name );
	}
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( esc_html__( 'Search results for', 'cosmos' ).sprintf( ' &ldquo;%s&rdquo;' , get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( esc_html__( 'Page', 'cosmos' ).' '.get_query_var( 'paged' ) );
		}
	}
}