...
class Cosmos_Abstract {
	...
	public function save() {
		...
		foreach( func_get_args() as $field ) {
			$new = Cosmos::get_request_param( $field, NULL );
	...
	public function upload_video($name,$value='',$label, $desc = '')
	{
		...
		$output .= '<div class="upload">';
			$output .= '<div class="upload-video">';
				...
				$output .= '<div class="button-upload"><input class="pix_upload_button" type="button" value="Browse" /></div>';
			$output .= '</div>';
		$output .= '</div>';
	...
	public function tooltip_html( $content, $position = 'right' ) {
		echo '<a href="javascript:void(0);" class="pixcore-tooltip" tabindex="-1" data-position="' . $position . '" data-content-as-html="true" title="' . esc_attr($content) . '">?</a>';
	}
	/**
	...
	public function gallery( $name, $images_id ) {
		echo '<div id="pixcore_gallery_container">
				...
				if( !empty($id) ){
					...
					if($image_src){
						printf('<li class="image" data-attachment_id="%s">
								...
								$id,
								$image_src[0],
								esc_html__('Delete image', 'cosmos')
							);
					}
				}
			}
		...
		echo '	</ul>';
		printf('<input type="hidden" id="pixcore_gallery_image_ids" name="%s" value="%s" />',
				 esc_attr($name),
				 esc_attr($images_id)
			);
		echo '</div>';
		...
		printf('<a href="#" data-delete="%2$s" data-title="%3$s" data-btn-text="%4$s" class="btn-open-gallery" title="%1$s">%1$s</a>',
				esc_html__('Add gallery images', 'cosmos'),
				esc_html__('Delete image', 'cosmos'),
				esc_html__('Gallery Images', 'cosmos'),
				esc_html__('Add to gallery', 'cosmos')
			);
		echo '</p>';
	}
	/**
	...
	public function upload_attachment( $name, $attachments_ids ) {
		echo '<div id="pixcore_attachment_container">
				...
				if( !empty($id) ){
					...
					if($image_src){
						printf('<li class="image" data-attachment_id="%1$s">
								...
								esc_attr( $file_name ),
								esc_html( $type ),
								esc_html__('Remove  attachment', 'cosmos'),
								esc_html__('Remove', 'cosmos'),
								esc_url( $url )
							);
					}
				}
		...
		echo '	</ul>';
		printf('<input type="hidden" id="pixcore_attachment_image_ids" name="%s" value="%s" />',
				 esc_attr($name),
		...
		printf('<a href="#" data-delete="%2$s" data-title="%3$s" data-btn-text="%4$s" class="btn-open-attachment button" title="%1$s">%1$s</a>',
				esc_html__('Add Media', 'cosmos'),
				esc_html__('Remove', 'cosmos'),
				esc_html__('Attachments', 'cosmos'),
				esc_html__('Add media', 'cosmos')
			);
		echo '</p>';
	}
	public function single_image( $name, $value, $html_options = array() ) {
		...
		$output = '<div class="screenshot  pix-image-upload-wrapper '.$img_show.'" ><img src="'.esc_url($value).'"/></div>';
		$output .= '<div class="screenshot  pix-no-image '.$no_img_show.'" ></div>';
		$img_id_name = $name . '_id';
		if( !empty( $prefix ) ) {
			$img_id_name = $prefix . '[' . $img_id_name . ']';
		}
		$output .= '
					<input type="button" data-rel="' . $html_options['data-rel'] .'" class="button pix-image-upload-btn pix-btn-upload " value="'. esc_html__( 'Upload Image', 'cosmos' ) .'" />
					<input type="button" data-rel="' . $html_options['data-rel'] .'" class="button pix-image-upload-btn pix-btn-remove '.$img_show.'" value="'. esc_html__( 'Remove', 'cosmos' ) .'" />
				';
