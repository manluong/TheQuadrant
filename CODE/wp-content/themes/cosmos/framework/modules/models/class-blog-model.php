...
class Cosmos_Blog_Model {
	...
	public function set_attributes( $atts = array() ) {
		...
		$default = array(
			...
			'pagination'           => '',
			'category_filter'      => '',
			'category_filter_text' => esc_html__( 'All', 'cosmos' ),
			'title_length'         => '',
			'excerpt_length'       => '',
			'show_category'        => '',
			'show_tag'             => '',
		...
		// query
		if ( !empty($this->attributes['query_related']) ) {
			$this->query = $this->get_query_related_posts( get_the_id(), $data );
		} else {
	...
	private function get_query( $data, $paged = '') {
		...
		switch ( $data['sort_by'] ) {
			...
			case 'za_order':
				...
				break;
			case 'popular':
				$query_args['meta_key'] = 'cosmos_postview_number';
	...
	public function get_views( $echo= false ) {
		...
		if( $this->attributes['show_views'] != 'hide' ) {
			...
			$out = sprintf( $format, esc_html__('View', 'cosmos'), esc_html( $viewcount ) );
		}
		if( ! $echo ) {
			return $out;
		}
	 ...
	 * @return string
	 */
	public function get_comments( $echo = false, $html = '', $validate = true ) {
		$out = '';
		if(!empty($html)){
			$format = $html;
		} else {
		...
		if( $this->attributes['show_comments'] != 'hide' ) {
			if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				$out = sprintf( $format, esc_html__('Comment', 'cosmos'), esc_html( get_comments_number() ) );
			}
		}
		if ( ! $validate ) {
			$out = sprintf( $format, esc_html__('Comment', 'cosmos'), esc_html( get_comments_number() ) );
	...
	public function get_date( $echo = false, $not_div = false ) {
		...
		if ( $this->show_widget_meta ){
			if( $not_div ){
				$format ='<a class="description" href="%1$s">%2$s</a>';
			}else{
				$format ='<div class="date-created item"><a class="text" href="%1$s">%2$s</a></div>';
			}
		}
	 ...
	 * @return string
	 */
	public function get_category( $html_option = array(), $echo = false, $prefix_category = '' ) {
		$out = $cat = '';
		if( $this->attributes['show_category'] != 'hide' ) {
			...
			// Read the post meta
			$post_options = get_post_meta( $this->post_id, 'cosmos_post_options', true);
			...
			if ( $cat ) {
				$out = sprintf( $format, esc_url(get_category_link($cat->cat_ID)), esc_html( $cat->name ), esc_html($prefix_category) );
			}
		}
		if( !$echo ) {
			return $out;
	...
	public function get_featured_image( $thumb_type = 'large', $echo = false, $options = array() ) {
		...
		if( isset( $options['has_no_image'] ) && $options['has_no_image'] == false && ! $this->has_thumbnail ) {
			return;
		}
		...
		if( $this->has_thumbnail ) {
			...
			$alt = trim(strip_tags( $this->post->post_excerpt ));;
			if ( empty( $alt ) ) {
				$alt = trim(strip_tags( $this->post->post_title ));
			}
			$def_img = '';
			if( isset($this->attributes['thumb-size']['no-image-' . $thumb_type])) {
				$no_image = $this->attributes['thumb-size']['no-image-' . $thumb_type];
			} else {
				$no_image = $this->attributes['thumb-size']['no-image'];
			}
			$thumb_img = sprintf('<img src="%1$s" alt="%2$s" class="img-responsive" />', COSMOS_NO_IMG_URI . $no_image ,$alt);
		}
		$format_icon = Cosmos_Params::get( 'post-format-icon' );
		...
		$out = sprintf( $out, esc_url($this->permalink), $thumb_img, $img_cate, $options['thumb_href_class'] );
		if( !$echo ) {
			return $out;
		}
		echo wp_kses_post($out);
	 ...
	 * 
	 */
	public function get_featured_images( $thumb_type = 'large', $echo = false, $options = array() ) {
		$out = $thumb_img = $img_cate = '';
		$thumb_size = $this->attributes['thumb-size'][$thumb_type];
		...
		if( $this->has_thumbnail ) {
			$thumb_id = get_post_thumbnail_id( $this->post_id );
			$thumb_img = wp_get_attachment_image( $thumb_id, $thumb_size, false, array('class' => 'media-object') );
		} else {
			$alt = trim(strip_tags( $this->post->post_excerpt ));;
			if ( empty( $alt ) ) {
				$alt = trim(strip_tags( $this->post->post_title ));
			...
			$thumb_img = sprintf('<img src="%1$s" alt="%2$s" />', COSMOS_NO_IMG_URI . $no_image ,$alt);
		}
		$format_icon = Cosmos_Params::get( 'post-format-icon' );
		$post_format = get_post_format( $this->post_id );
		if( isset($format_icon[$post_format]) ) {
	...
	public function get_title( $is_limit = true, $echo = false, $options = array() ) {
		...
		if( ! $echo ) {
			return sprintf( $output, esc_html( $title ), esc_url($this->permalink), esc_html( $this->title ), esc_attr( $options['title_class'] ) );
		} 
		printf( $output, esc_html( $title ), esc_url($this->permalink), esc_html( $this->title ), esc_attr($options['title_class'] ) );
	}
	/**
	 * Get the permalink
	 */
	...
	public function view_more_button( $echo = false, $post_id = '', $btn_content = '', $html_options = array() ) {
		if( empty( $btn_content ) ) {
			$btn_content = esc_html__( 'View more', 'cosmos' );
		}
	...
	public function get_thumb_sizes( $sizes, $options= array(), $theme_prefix = 'cosmos' ) {
		$thumb_size = array(
			'large' => 'full',
		);
		...
		if( $sizes ) {
			foreach( $sizes as $key => $value ) {
				$prefix = 'thumb-';
				$ext = '.gif';
				if( $key == 'large' || $key == 'small' || $key == 'medium' || $key == $small_column ) {
	...
	public function get_featured_image_only( $thumb_type = 'large', &$options = array() ) {
		...
		if( $thumb_img ) {
			$out = sprintf( $format, esc_url($this->permalink), $thumb_img, $thumb_href_class );
		}
		return $out;
	}
	public function get_featured_by_format( $thumb_type = 'large', &$options = array() ) {
		...
		if( $this->isVideo( $this->post_id ) && class_exists('Cosmos_Core_Video_Model')) {
			//video
			$model = new Cosmos_Core_Video_Model();
			$feature_video = $model->get_feature_video( $this->post_id, 'cosmos_feature_video' );
			if( $featured_image ) {
				$options['bg_image'][$this->post_id] = $opts['ret_attach_img'][0];
			}
			if( $feature_video && $featured_image && !empty($options['video_format'])) {
				return sprintf( $options['video_format'], $feature_video, $featured_image );
			}
		} else {
			//feature image
			if( $featured_image ) {
